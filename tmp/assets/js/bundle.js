...
var view=document.getElementById('view-main');var scrollbar=smooth_scrollbar__WEBPACK_IMPORTED_MODULE_0__["default"].init(view,{renderByPixels:false,damping:0.075});// Header out trigger animation
(function(){var container=document.querySelector('.c-demo');var content=container.querySelector('.c-demo-content');var tl=new gsap__WEBPACK_IMPORTED_MODULE_3__["default"].timeline({scrollTrigger:{trigger:container,scrub:true,start:"bottom center"}});tl.to(content,{y:"50%",skewY:3,duration:1,ease:"none"},0);tl.to(content,{opacity:0,duration:0.5,ease:"none"},0);})();// Sequence trigger animation
(function(){// generate array of images paths. length = frames length, see: /src/assets/img
var urls=_toConsumableArray(new Array(162)).map(function(value,index){return"img/".concat(index+1,".jpg");});// load images async
var images=Object(utills__WEBPACK_IMPORTED_MODULE_4__["preloadImages"])(urls);var container=document.querySelector('.c-sequence');var canvas=container.querySelector('canvas');var ctx=canvas.getContext('2d');// create "scrub" ScrollTrigger effect with pin of main block
...
pin:true,pinType:"transform"}});// canvas resize handler
window.addEventListener('resize',function resize(){ctx.canvas.width=document.documentElement.clientWidth;ctx.canvas.height=document.documentElement.clientHeight;return resize;}());// when all images ready
...
onUpdate:function onUpdate(){return Object(utills__WEBPACK_IMPORTED_MODULE_4__["calcDrawImage"])(ctx,imgs[counter.i]);}// draw image in canvas when timeline update
},0);// draw current frame again when scroll stopped and resize happened
window.addEventListener('resize',function(){return Object(utills__WEBPACK_IMPORTED_MODULE_4__["calcDrawImage"])(ctx,imgs[counter.i]);});});})();
 ...
 */var calcDrawImage=function calcDrawImage(ctx,image){var left=arguments.length>2&&arguments[2]!==undefined?arguments[2]:0.5;var top=arguments.length>3&&arguments[3]!==undefined?arguments[3]:0.5;var cWidth=ctx.canvas.width;var cHeight=ctx.canvas.height;var width=image.width;var height=image.height;var ratio=width/height;var cRatio=cWidth/cHeight;var resultHeight,resultWidth;if(ratio>cRatio){resultHeight=cHeight;resultWidth=cHeight*ratio;}else{resultWidth=cWidth;resultHeight=cWidth/ratio;}ctx.drawImage(image,(cWidth-resultWidth)*left,(cHeight-resultHeight)*top,resultWidth,resultHeight);};
 ...
 * @version 1.0.2
 * @author Michael Gudzevskyi (Draft) 
 */var SoftScrollPlugin=/*#__PURE__*/function(_Scrollbar$ScrollbarP){_inherits(SoftScrollPlugin,_Scrollbar$ScrollbarP);var _super=_createSuper(SoftScrollPlugin);function SoftScrollPlugin(){_classCallCheck(this,SoftScrollPlugin);return _super.apply(this,arguments);}_createClass(SoftScrollPlugin,[{key:"transformDelta",value:function transformDelta(delta,fromEvent){var dirX=delta.x>0?1:-1;var dirY=delta.y>0?1:-1;if(dirX===this.lockX||dirY===this.lockY){return{x:0,y:0};}else{this.lockX=null;this.lockY=null;}return delta;}},{key:"onRender",value:function onRender(Data2d){var x=Data2d.x,y=Data2d.y;// Up
